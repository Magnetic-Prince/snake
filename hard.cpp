...
void SnakeNode3::setPositionRC(int row, int col)//设置节点的坐标
{
    this->snake_row = row;
    this->snake_col = col;
...
SnakeNode3* SnakeNode3::create(Part3 type)//应用init
{
...
bool SnakeNode3::init(Part3 type)//创建蛇头，蛇身，以及食物
{
    if (!Sprite::init())
    {
        return false;
    ...
    case Part3::HEAD:
    {
        ...
        snake_dir = ENUM_DIR3::RIGHT;//初始化移动方向，向右移动
...
void Game_3::newBody()//添加一个新的身体节点
{
...
void Game_3::moveBody()//移动所有的身体节点
{
    if (0 == allBody.size())
    {
        return;
